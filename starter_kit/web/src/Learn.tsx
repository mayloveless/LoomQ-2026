import { GlossaryProvider, Term, type GlossaryTerm } from './Glossary'

type LearnScreenProps = {
  onStart: () => void
}

const RECAP: Array<[GlossaryTerm, string]> = [
  ['qubit', '程序正在操作的数据单位'],
  ['state', '程序此刻保留的量子可能性'],
  ['gate', '改变当前量子状态的操作'],
    ...
    <GlossaryProvider>
      ...
      <section className="learn-hero">
        <div className="learn-hero-copy">
          ...
          <h1>跟着一个 <Term term="qubit">qubit</Term>，<br />看懂量子程序怎么运行</h1>
          <p>不需要量子物理背景。我们只运行一个真实的三步程序：准备<Term term="state">状态</Term>、用 H 改变状态，最后读取结果。</p>
      ...
      <section className="learn-program" id="guided-program" aria-labelledby="program-heading">
        <div className="learn-section-heading">
          <div>
            ...
            <h2 id="program-heading">一个 <Term term="circuit">quantum circuit</Term>，三步看完</h2>
          </div>
          <p>先拿到程序执行的整体框架，再观察每条指令让状态发生了什么。</p>
        </div>
        <div className="learn-circuit-anchor">
          <p><strong><Term term="circuit">quantum circuit</Term></strong> 可以先理解成一段按顺序执行的程序。</p>
          <div aria-label="量子电路的三步执行框架">
            <span>准备<Term term="state">状态</Term></span><i>→</i><span><Term term="gate">gate</Term> 改变状态</span><i>→</i><span><Term term="measurement">measurement</Term> 读取结果</span>
          </div>
          <small>下面用一个 <Term term="qubit">qubit</Term>，真正按这个顺序运行一遍。</small>
        </div>
        ...
        <div className="learn-step-list">
          <article className="learn-program-step">
            ...
            <div className="learn-step-copy">
              ...
              <h3>准备一个 <Term term="qubit">qubit</Term></h3>
              <p>程序先准备一个 qubit。可以暂时把它理解成量子程序正在操作的数据单位；它一开始处于确定的 <code>|0⟩</code> <Term term="state">状态</Term>。</p>
          ...
          <article className="learn-program-step learn-step-featured">
            ...
            <div className="learn-step-copy">
              ...
              <h3>执行 H，状态发生变化</h3>
              <p><code>H</code> 是一个 <Term term="gate">quantum gate</Term>。gate 像一次状态变换：它把原本确定的 <code>|0⟩</code>，变成同时保留两种可能性的状态。</p>
              <code className="learn-qasm-line">h q[0];</code>
              <aside className="learn-superposition-note">
                <strong>这就叫<Term term="superposition" marker>叠加 · superposition</Term></strong>
          ...
          <article className="learn-program-step">
            ...
            <div className="learn-step-copy">
              ...
              <h3>测量，把结果交给普通程序</h3>
              <p><Term term="measurement">measurement</Term> 把量子状态读取成经典的 <code>0</code> 或 <code>1</code>。一次运行只得到其中一个结果；重复运行很多次，才会看到接近 50% / 50% 的统计分布。</p>
            ...
            <div className="learn-measurement-card" aria-label="一次测量与重复运行的区别">
              ...
              <div><span>重复 1,000 <Term term="shots">shots</Term></span><strong>≈ 50% <i>/</i> 50%</strong><p>多次结果汇总成统计分布</p></div>
            </div>
          </article>
        </div>
      </section>
      ...
      <section className="learn-recap" aria-labelledby="recap-heading">
        <div className="learn-recap-intro">
          ...
          <h2 id="recap-heading">你刚刚已经读完了第一个 <Term term="circuit">quantum circuit</Term></h2>
          <p>现在只是给刚才已经看懂的内容补上名字。每个术语，都能在这段程序的执行过程中找到位置。</p>
        </div>
        <dl className="learn-recap-list">
          {RECAP.map(([term, copy]) => (
            <div key={term}><dt><Term term={term}>{term}</Term></dt><dd>{copy}</dd></div>
          ))}
        </dl>
      </section>
      ...
      <details className="learn-technical">
        ...
        <div className="learn-technical-grid">
          ...
          <p><code><Term term="shots">shots</Term></code><span>重复运行和测量，会得到 counts 与概率直方图。</span></p>
        </div>
      </details>

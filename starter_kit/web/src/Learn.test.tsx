...
describe('Task 13J compact three-step Learn experience', () => {
  it('positions why quantum, who LoomQ serves, and how it helps before the compact experience', () => {
    ...
    expect(text).toContain('从理解到执行，一条连续的实验路径')
    expect(text).toContain('01 · LEARN从状态变化开始理解')
    expect(text).toContain('02 · EXPERIMENT选择现象来探索')
    expect(text).toContain('03 · CREATE用 AI 创建并验证')
    expect(text).toContain('04 · EXECUTE沿执行过程理解结果')

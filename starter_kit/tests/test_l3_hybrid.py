...
def render_expression(expression: TExpression) -> str:
    ...
    return "(%s %s %s)" % (
        render_expression(expression.left),
        expression.operator,
        render_expression(expression.right),
    )


def render_statements(statements: Sequence[TStatement], indent: str = "  ") -> str:
    lines: List[str] = []
    for statement in statements:
        if isinstance(statement, TAssignment):
            lines.append(
                "%sr%d = %s;"
                % (indent, statement.target, render_expression(statement.expression))
            )
        else:
            lines.append(
                "%sif (%s %s %s) {"
                % (
                    indent,
                    render_expression(statement.left),
                    statement.operator,
                    render_expression(statement.right),
                )
            )
            lines.append(render_statements(statement.then_body, indent + "  "))
            lines.append("%s} else {" % indent)
            lines.append(render_statements(statement.else_body, indent + "  "))
...
class HiddenLikeDifferentialTests(unittest.TestCase):
    def test_fixed_seed_random_programs_match_reference_interpreter(self) -> None:
        ...
        self.assertEqual(120, program_count)
        self.assertEqual(930, input_count)
